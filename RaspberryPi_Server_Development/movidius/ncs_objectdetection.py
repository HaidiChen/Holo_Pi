...
def detectFromImage(imageFile):

    # define necessary parameters
    graphPath = "graphs/mobilenetgraph"
    confidence = 0.6
    detectionResults = []
		...
		if pred_conf > confidence:
			...
			cv2.putText(image_for_process, label, (startX, y),
				cv2.FONT_HERSHEY_SIMPLEX, 1, COLORS[pred_class], 3)
                        ...
                        d = datetime.now()
                        tp = d.timetuple()
                        directory = os.mkdir('Images/DetectionResults/' + d.date().isoformat())
                        fileName = str(tp[3]) + '-' str(tp[4]) + '-' + str(tp[5]) + '.jpg'
                        ...
                        detection_dict["ItemDescription"] = "This is supposed to be a description of this detected item."
